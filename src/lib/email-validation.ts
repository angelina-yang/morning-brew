...
// Common disposable / test / example domains.
// Kept intentionally short — the goal is to stop lazy fakes, not wage war on tempmail.
...
// Obvious placeholder local parts.
...
export function isValidEmail(raw: string): boolean {
  ...
  // Basic shape: local@domain.tld (TLD at least 2 chars)
  ...
  // "aaaa@anything.com" or single-char local repeated
  if (/^(.)\1{2,}$/.test(local)) return false;

  // Domain must have a valid-looking TLD (letters only, 2+ chars)
